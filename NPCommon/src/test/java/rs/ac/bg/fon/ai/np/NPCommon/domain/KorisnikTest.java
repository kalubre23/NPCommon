...
class KorisnikTest {
	...
	@Test
	void testSetKorisnikIDManjiOdNula() {
		assertThrows(IllegalArgumentException.class, ()->s.setKorisnikID(-1));
	}
	
	@Test
	void testSetKorisnikIDOk() {
		s.setKorisnikID(1);
		assertEquals(1, s.getKorisnikId());
	}
	...
	void testSetUlogaNull() {
            assertThrows(NullPointerException.class, ()->s.setUloga(null));
	}
        
        @Test
	void testSetUlogaOk() {
            Uloga u = new Uloga(2, "serviser");
            s.setUloga(u);
            assertEquals(u, s.getUloga());
	}
